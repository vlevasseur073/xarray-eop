...
from xarray_eop.verification.logger import get_logger, get_failed_logger, get_passed_logger
from xarray_eop.sentinel3 import open_safe_datatree
from xarray_eop.sentinel3 import open_sentinel3_dataset
from xarray_eop.eop import open_eop_dataset
...
def encode_time_dataset(ds:xr.Dataset)->xr.Dataset:
    for name,var in ds.data_vars.items():
        if var.dtype == np.dtype("timedelta64[ns]") or var.dtype == np.dtype("datetime64[ns]"):
...
def compare(
    ...
    logger.info(f"Compare the new product {new.name} to the reference product {reference.name}")
    ...
    if reference.suffix == ".SEN3":
        ...
        dt_ref = open_safe_datatree("ref",reference, simplified_mapping=simplified_mapping)
...
def call_compare(reference: str, new: str):
    runner = CliRunner()
    result = runner.invoke(compare, ['--reference', reference, '--new', new, '--verbose','--relative',"--threshold",1e-6])
...
if __name__ == "__main__":
    # Test
    internal_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Products")
    ref = internal_path / "SAFE" / "S3B_OL_1_ERR____20230506T015316_20230506T015616_20230711T065804_0179_079_117______LR1_D_NR_003.SEN3"
