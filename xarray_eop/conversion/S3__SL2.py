...
out_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr_new/SL2")
tmp_path =  Path("/tmp")
...
def generate(adf_type: str)->None:
    ...
    if adf_type == "SLLCC":
        for legacy_adf in ["S3A_SL_1_LCC_AX","S3B_SL_1_LCC_AX"]:
            ...
            dt = generate_datatree_from_legacy_adf(safe,title="land cloud coefficient data file")
            dt.to_zarr(out_file)
    ...
    if adf_type == "LSTCD":
        for legacy_adf in ["S3A_SL_2_LSTCAX","S3B_SL_2_LSTCAX"]:
            ...
            dt = generate_datatree_from_legacy_adf(
                ...
                group="",
                var_to_attr={"skip_biomes": ["long_name","flag_meanings"],},#"time_bounds": []}
            )
            dt.to_zarr(out_file)
    ...
    if adf_type == "SLIRE":
        ...
        old_zarr_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr/SL2")
    ...
    if adf_type == "IMSCD":
        for legacy_adf in ["S3__SL_2_IMSCAX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format = 'zarr')
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "LSTWV":
        ...
        for adf in files:
            ...
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
            dt = datatree.open_datatree(files[0],engine="zarr",chunks={},decode_times=False)
            # maps = xr.open_dataset(files[0]/"maps",engine="zarr",chunks={},consolidated=False)
            # c = xr.open_dataset(files[0]/"coordinates",engine="zarr",chunks={},consolidated=False)
            # ds = xr.open_dataset(files[0],engine="zarr",chunks={},consolidated=False)
            # print(dt)
