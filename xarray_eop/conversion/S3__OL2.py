...
out_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr_new/OL2")
tmp_path =  Path("/tmp")
...
def generate(adf_type: str)->None:
    ...
    if adf_type == "OLACP":
        for legacy_adf in ["S3A_OL_2_ACP_AX","S3B_OL_2_ACP_AX"]:
            ...
            out_file = out_path / out_file
            dt = datatree.DataTree(name="root")
            for ncgroup in ["glint_whitecaps","bright_waters_NIR","standard_AC","alternate_AC",]:
    ...
    if adf_type == "OLOCP":
        for legacy_adf in ["S3A_OL_2_OCP_AX","S3B_OL_2_OCP_AX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "OLVGP":
        for legacy_adf in ["S3A_OL_2_VGP_AX","S3B_OL_2_VGP_AX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "OLCLP":
        for legacy_adf in ["S3A_OL_2_CLP_AX","S3B_OL_2_CLP_AX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
