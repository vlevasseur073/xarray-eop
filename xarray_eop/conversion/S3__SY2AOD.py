...
out_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr_new/SY2_AOD")
tmp_path =  Path("/tmp")
...
def generate(adf_type: str)->None:
    ...
    if adf_type == "AODRT":
        for legacy_adf in ["S3__SY_2_ART_AX",]:
    ...
    if adf_type == "AODOR":
        for legacy_adf in ["S3__SY_2_OSR_AX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "AODLR":
        for legacy_adf in ["S3__SY_2_LSR_AX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "AODAC":
        for legacy_adf in ["S3__SY_2_ACLMAX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
    ...
    if adf_type == "SYAOD":
        for legacy_adf in ["S3__SY_2_AODCAX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            ...
            for envi_file in sorted(safe.glob("*.img")):
                ...
                # print(rds)
                data.append(rds.sel(band=1).drop_vars("band").rename("aod550"))#.rename({"band_data":"aod550"}))
