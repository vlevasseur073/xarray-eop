...
out_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr_new/SL1")
tmp_path =  Path("/tmp")
...
def generate(adf_type: str)->None:
    ...
    if adf_type == "SLGEC":
        merged_variables = ['quaternions_1','quaternions_2','quaternions_3','quaternions_4']
        merged_mapping = {
            'quaternions' :(
                merged_variables,
                "n_quaternions",
        ...
        for legacy_adf in ["S3A_SL_1_GEC_AX","S3B_SL_1_GEC_AX"]:
            ...
            dt:datatree.DataTree = generate_datatree_from_legacy_adf(
                ...
                group="") # if group=None, by default subgroup maps is created if group=="" no subgroup
            dt.to_zarr(out_file)
    ...
    if adf_type == "SLGEO":
        for legacy_adf in ["S3A_SL_1_GEO_AX","S3B_SL_1_GEO_AX"]:
            ...
            dt:datatree.DataTree = generate_datatree_from_legacy_adf(
                ...
                group="")
            dt.scans_cone_angle.attrs["long_name"] = "Cone angle for each scan (nadir first, then oblique)"
            dt.to_zarr(out_file)
    ...
    if adf_type == "SLCDP":
        for legacy_adf in ["S3A_SL_1_CDP_AX","S3B_SL_1_CDP_AX"]:
            ...
            dt.d110120.attrs['long_name'] = 'Difference between BT (11\u03BCm) and BT (12\u03BCm)'
            dt.d037110.attrs['long_name'] = 'Difference between BT (3.7\u03BCm) and BT (11\u03BCm)'
            ...
            dt.c016.attrs['long_name'] = 'Reflectances on 1.6\u03BCm channel in cloudy conditions'
            dt.c008.attrs['long_name'] = 'Reflectances on 0.87\u03BCm channel in cloudy conditions'
            dt.c006.attrs['long_name'] = 'Reflectances on 0.66\u03BCm channel in cloudy conditions'
            dt.d006008.attrs['long_name'] = 'Differences betwen R(0.66\u03BCm) and R(0.87\u03BCm)'
            dt.tir_110_120.attrs['long_name'] = 'Spectral Probability density LUT for Thermal daytime (11\u03BCm, 12\u03BCm)'
            dt.txt_110.attrs['long_name'] = 'Probability density LUT for day- and night-time thermal texture (LSD 11\u03BCm)'
            dt.tir_037_110_120.attrs['long_name'] = 'Probability density LUT for Thermal nighttime (3.7\u03BCm, 11\u03BCm, 12\u03BCm)'
            dt.vis_006_008.attrs['long_name'] = 'Spectral Probability density LUT associated with Joint daytime reflectance in cloudy conditions'
            dt.to_zarr(out_file)
    ...
    if adf_type == "SLCLP":
        for legacy_adf in ["S3A_SL_1_CLP_AX","S3B_SL_1_CLP_AX"]:
            ...
            dt.solz.attrs['long_name'] = 'Solar Zenith Angle (reduced resolution)'
            dt.t110.attrs['long_name'] = 'textural BT 11\u03BCm channel'
            dt.txt_110.attrs['long_name'] = 'Probability density LUT for thermal texture (LSD 11\u03BCm)'
            dt.to_zarr(out_file)
            ...
            for adf in legacy_ADF[sat]:
                ...
                dt = generate_datatree_from_legacy_adf(
                    ...
                    coordinates_variable={ # variable : (dimention,coordinate)
                        ...
                        "cal_uncertainty_uncertainty": ("uncertainty_lut","cal_uncertainty_temperature"),
        ...
        for sat in ["S3A","S3B"]:
            ...
            for adf in legacy_ADF[sat]:
                ...
                dt = generate_datatree_from_legacy_adf(
                    ...
                    coordinates_variable={ # variable : (dimention,coordinate)
                        "cal_uncertainty_uncertainty": ("uncertainty_lut","cal_uncertainty_radiance"),
    ...
    if adf_type == "SLBDF":
        ...
        old_zarr_path = Path("/mount/internal/work-st/projects/cs-412/2078-dpr/Samples/Auxiliary/Zarr/SL1")
