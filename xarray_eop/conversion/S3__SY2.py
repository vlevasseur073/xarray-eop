...
def generate(adf_type: str)->None:
    ...
    if adf_type == "SYCDI":
        for legacy_adf in ["S3__SY_1_CDIBAX",]:
            safe = extract_legacy(in_path,legacy_adf,tmp_path)
            out_file = gen_static_adf_name(legacy_adf[0:3],adf_type,format="zarr")
            print(_LOG_GENERATING_ADF+out_file)
            out_file = out_path / out_file
            ...
            for j in range(8):
                ...
                for i in range(4):
                    ...
                    var.append(xr.open_dataset(safe/tif_file,chunks={"x":4050,"y":4050}).squeeze(drop=True))
        ...
        for adf in ancillary_adf:
            ...
            for legacy_adf in ancillary_adf[adf][0]:
                ...
                for nc_file in safe.glob("*.nc4"):
                    ...
                    ds=xr.concat(ds,xr.DataArray(band_index,name=f"{group}_band",dims="band"))
            ...
            dt.attrs.update(
                {
                    "title": title,
                    "baseline_collection" : f"{baseline_collection:03d}"
                }
            )
            dt.to_zarr(out_file)
